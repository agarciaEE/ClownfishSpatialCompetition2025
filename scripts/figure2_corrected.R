...
data <- data[!(data$species == "Amphiprion_bicinctus" & data$region == "Somali_Arabian"),]
data <- data[!(data$species == "Amphiprion_percula" & data$region == "Southeast_Polynesia"),]
write.csv(data, "./results/ROU_data_corrected.csv", row.names = FALSE)
...
data[order(data$Restricted, decreasing = TRUE),]
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
ggsave("figures/Fig2.speciesROU_and_ROU_vs_behavior_corrected.pdf", plot = fig2, width=18, height=12, units="in")
