...
# Load ROU estimates
data <- read.csv("../tests/results_new/ROU_data_oldA.csv")
...
int.mat = read.csv("../tests/interaction_matrix_old.csv", row.names = 1)
int.mat[int.mat > 0] = 1

mean(data$Restricted, na.rm = TRUE)
mean(data$Occupied, na.rm = TRUE)
mean(data$Unexploited, na.rm = TRUE)
...
ggsave("figures/Fig2.speciesROU_and_ROU_vs_behavior_oldmatrix.pdf", plot = fig2, width=18, height=12, units="in")
