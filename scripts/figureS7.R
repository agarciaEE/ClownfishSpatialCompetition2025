...
SR_legend_data <- expand.grid(amph_richness = seq(0,max_amph, 0.1), anem_richness = seq(0, max_anem, 0.1))

# Apply the function to generate colors
SR_legend_data <- SR_legend_data %>%
  mutate(color = richness_palette(amph_richness, anem_richness, max_amph, max_anem))

# Create 2D Color Legend (Richness Gradient)
SR_legend <- ggplot(SR_legend_data, aes(x = anem_richness, y = amph_richness, fill = color)) +
  geom_tile() +
  ...
  theme_minimal() +
  labs(y = "Clownfish Richness", x = "Sea Anemone Richness") +
  theme(
...
# Create a grid of values for the color scale
NO_legend_data <- expand.grid(ENTotal = seq(0, max_a, 0.01), ROTotal = seq(0, max_b, 0.01))

# Apply the function to generate colors
NO_legend_data <- NO_legend_data %>%
  dplyr::mutate(color = mapply(generate_2D_gradient, ENTotal, ROTotal))

# Create 2D Color Legend (Overlap Gradient)
NO_legend <- ggplot(NO_legend_data, aes(x = ROTotal, y = ENTotal, fill = color)) +
  geom_tile() +
  geom_abline(color = "white", linewidth = 1, linetype = "dashed") +
...
for (i in 1:length(interaction_types)) {
  ...
  if (int == "GG") {
    ...
    sr_val = "amph.richness"
    en_val = "ENGS"
    ro_val = "ROGS"
  }
  
  INT.pred <- INT.pred %>%
    dplyr::mutate(color = richness_palette(get({{sr_val}}), anem.richness, max_amph, max_anem))
  ...
  SR_top_margin <- ggplot(SR_longitude_agg, aes(x = bin)) +
    ...
    theme_void() +
    theme(plot.margin = margin(0, 0, 0, 280),
          panel.grid.major = element_line(color = "grey90"),
          panel.background = element_rect(fill ="white", color = NA))
  
  
  # Right margin plot: Density of species richness along latitude
  SR_right_margin <- ggplot(SR_latitude_agg, aes(x = bin)) +
    geom_area(aes(y = amph.richness), fill = "#D96666", alpha = 0.5) +
    geom_area(aes(y = anem.richness), fill = "#6699D9", alpha = 0.5) +
    theme_void() +
  ...
  if (int == "GG") { SR_legend <- SR_legend +  labs(y = "Number of Generalists", x = "Sea Anemone Richness") }
  else if (int == "SS") { SR_legend <- SR_legend +  labs(y = "Number of Specialists", x = "Sea Anemone Richness") }
  else { SR_legend <- SR_legend +  labs(y = "Clownfish Richness", x = "Sea Anemone Richness") }
  ...
  INT.pred <- INT.pred %>%
    dplyr::mutate(color = mapply(generate_2D_gradient, get({{en_val}}), get({{ro_val}})))
  ...
  # Create aggregated data for margins
  NO_longitude_agg <- INT.pred %>%
    dplyr::group_by(bin = round(x)) %>%
    dplyr::summarise(ENTotal = mean(get({{en_val}}), na.rm = TRUE),
                     ROTotal = mean(get({{ro_val}}), na.rm = TRUE))
  
  NO_latitude_agg <- INT.pred %>%
    dplyr::group_by(bin = round(y)) %>%
    dplyr::summarise(ENTotal = mean(get({{en_val}}), na.rm = TRUE),
                     ROTotal = mean(get({{ro_val}}), na.rm = TRUE))
  
  # Top margin plot: Density of species richness along longitude
  NO_top_margin <- ggplot(NO_longitude_agg, aes(x = bin)) +
  ...
  # Right margin plot: Density of species richness along latitude
  NO_right_margin <- ggplot(NO_latitude_agg, aes(x = bin)) +
    geom_area(aes(y = ENTotal), fill = "#D96666", alpha = 0.5) +
    geom_area(aes(y = ROTotal), fill = "#6699D9", alpha = 0.5) +
    theme_void() +
    scale_x_continuous(breaks = seq(-30, 30, 10), expand = c(0,0)) +
  ...
  p[[int]] <- ggarrange(SRplot, Overlap_plot, nrow = 1, labels = paste0("(", letters[i], ")"), 
                     font.label = list(size = 30), label.x = 0.1)
...
figS7 <- ggarrange(p$GG, empty_plot, p$GS, empty_plot, p$SS, 
                   nrow = 5, heights = c(1, 0.1, 1, 0.1, 1))

ggsave("figures/FigS7.geographical_richness_overlap_by_interaction_type.pdf", 
       plot = figS7, width=48, height=30, units="in", limitsize = FALSE)
